...
void CCyichang::userInput(HANDLE hPorc, HANDLE hTread)
{
	...
	while (true)
	{
		...
		else if (strcmp(cmd, "dll") == 0) {    //查看反汇编数据	
											 // 输出调试信息
			showalldll();
			//setAllBreakpoint(this->hProc);
			this->userInput(this->hProc, this->hThread);
		...
		else if (strcmp(cmd, "cpp") == 0) {    //查看源码
			ccLook();
			Symbol::Init(this->hProc, this->hThread, cmd);
			Symbol::ShowSource(1, 10);
			this->userInput(this->hProc, this->hThread);
			break;
		}
...
/////////////
	if (SymInitialize(ccmyhproc, "C:\\Users\\1234\Downloads\\securityguard-master\\任务管理器5.0\\Debug\\MFCApplication2.pdb", FALSE))
	{
		...
		DWORD64 moduleAddress = SymLoadModule64(ccmyhproc,
			cppPsInfo.hFile, moduleFileName,
			NULL,
			(DWORD64)cppPsInfo.lpBaseOfImage, 0);
		if (moduleAddress == 0)
		{
			cout << "加载调试符号失败" << endl;
		}
...
///////////////
	...
	DWORD    dwDisplac = 0;
	char buffer2[sizeof(SYMBOL_INFO) + MAX_SYM_NAME * sizeof(TCHAR)];
	PSYMBOL_INFO pSymbol2 = (PSYMBOL_INFO)buffer2;
	IMAGEHLP_LINE64 line;
	SymSetOptions(SYMOPT_LOAD_LINES);
	pSymbol2->SizeOfStruct = sizeof(SYMBOL_INFO);
	pSymbol2->MaxNameLen = MAX_SYM_NAME;
	if (SymGetLineFromAddr64(this->hProc, dwAddress, &dwDisplac, &line)) {
		printf("源文件路径：%s\n", line.FileName);
		printf("第%d行\n", line.LineNumber);
	}
}
//查看所有模块信息
void CCyichang::showalldll()
{
	...
	LOAD_DLL_DEBUG_INFO pInfo;
	char szBuf[MAX_PATH] = { '\0' };
	SIZE_T nNumberOfBytesRead = 0;
	DWORD dwAddrImageName = 0;
	for (auto temp : this->dllinfo) {
		printf("HANDLE:            %d\n", temp.hFile);
		printf("模块载基址：       %08x\n", temp.lpBaseOfDll);
		printf("InfoFileOffset：   %08x\n", temp.dwDebugInfoFileOffset);
		printf("InfoSize：         %08x\n", temp.nDebugInfoSize);
		if (temp.lpImageName != NULL) {
		//printf("模块名称：         %s\n", temp.lpImageName);
		}
		...
		if ((NULL != &pInfo) && (NULL != pInfo.lpImageName)) {
			...
			do {
				  //VirtualProtectEx(this->hProc, lpAddr, 100, 0x20, &lpflOldProtect);
				bool bo = ReadProcessMemory(this->hProc, pInfo.lpImageName, &dwAddrImageName, sizeof(dwAddrImageName), &nNumberOfBytesRead);
				if (!bo) {
					break;
				}
				...
				if (!ReadProcessMemory(this->hProc, (void*)dwAddrImageName, &szBuf, sizeof(szBuf), &nNumberOfBytesRead)) {
					break;
				}
