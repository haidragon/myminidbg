...
BOOL CALLBACK SymCallback(PSYMBOL_INFO lpSymInfo, ULONG SymbolSize, PVOID UserContext)
{
	...
	if (lpSymInfo->Tag == SymTagData) {

		varInfo.address = Symbol::GetSymbolAddress(lpSymInfo);
		varInfo.modBase = (DWORD)lpSymInfo->ModBase;
		varInfo.size = SymbolSize;
		varInfo.typeID = lpSymInfo->TypeIndex;
		varInfo.name = lpSymInfo->Name;
...
DWORD Symbol::cmdShowLocalVariables()
{
	...
	if (SymSetContext(GetDebuggeeHandle(), &stackFrame, NULL) == FALSE) {
		...
		if (GetLastError() != ERROR_SUCCESS) {
			std::wcout << "No debug info in current address." << std::endl;
			return 1;
		}
	}
