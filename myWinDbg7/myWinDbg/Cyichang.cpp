...
void CCyichang::userInput(HANDLE hPorc, HANDLE hTread)
{
	...
	while (true)
	{
	    ...
	    //执行
			...
			//VirtualQueryEx(hPorc,pAddress, &mbi, sizeof(mbi));
			SIZE_T dwRead = 0;

			//设置改页可以读可写
		/*	const char* ch = stropcode.c_str();*/
			//char temp[] = "\x80\x90";
			...
			////恢复页都保护属性
			//VirtualProtectEx(hPorc, dwAddr, sizeof(this->opcode), dwOldProtect, 0);
			//设置单步
			/*setBreakpoint_tf(hTread);
			g_isUserTf = TRUE;*/
			setAllBreakpoint(this->hProc);
			...
			scanf_s("%x", &dwAddr);
			getchar();
			if (!ReadProcessMemory(hProc, dwAddr, opcode, 200, &read)) {
				DBGPRINT("读取内存失败\n");
			}
			else {
				DISASM disAsm = { 0 };
				...
				while (nSize < 5)
				{
					...
					printf("%X ",(byte)temp[i]);*/
					for (int j = 0;j < nLen;++j) {
						printf("%2X ", *(BYTE *)disAsm.VirtualAddr);
						disAsm.VirtualAddr += 1;
...
//#define PAGE_READWRITE         0x04
	DWORD dw;
	printf("什么权限的断点（访问 1，只读 2，可读可写 4）：");
	scanf_s("%x", &dw);
	getchar();
	int len;
	printf("长度：");
